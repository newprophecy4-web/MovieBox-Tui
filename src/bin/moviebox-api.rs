...
use moviebox_tui::{
    providers::{MediaType, ProviderError, ProviderKind},
...
fn decode_id(id: &str) -> Result<(ProviderKind, String), Response> {
    ...
    let (provider, value) = raw.split_once('|').ok_or_else(|| {
        error(
            StatusCode::BAD_REQUEST,
            "INVALID_REQUEST",
            "Malformed title ID",
...
fn provider_error(e: ProviderError) -> Response {
    let (status, code) = match e {
        ProviderError::NotFound => (StatusCode::NOT_FOUND, "NOT_FOUND"),
        ProviderError::RateLimited(_) => (StatusCode::TOO_MANY_REQUESTS, "RATE_LIMITED"),
        ProviderError::Network(_) => (StatusCode::BAD_GATEWAY, "UPSTREAM_ERROR"),
        ProviderError::Parsing(_) | ProviderError::Unavailable(_) => {
            (StatusCode::BAD_GATEWAY, "PROVIDER_ERROR")
        }
    };
    error(status, code, "Provider request failed")
}
fn media_type(t: MediaType) -> &'static str {
    match t {
...
fn detected_media_type(content_type: Option<&str>, final_url: &str) -> &'static str {
        ...
        || content_type.contains("application/x-mpegurl")
    {
        ...
        "dash"
    } else if content_type == "video/mp4" || content_type.starts_with("video/mp4;") {
        "mp4"
    } else {
        // URL suffixes are intentionally not used as evidence of media type.
        let _ = final_url;
        "unknown"
    }
}

fn safe_probe_url(raw: &str) -> bool {
...
async fn source_diagnostics(
    client: &reqwest::Client,
    source: &moviebox_tui::providers::models::PlaybackSource,
) -> serde_json::Value {
    if !safe_probe_url(&source.url) {
        return serde_json::json!({
            "resolved": true,
            "probe_error": "source URL was not eligible for diagnostic probing",
            "browser_compatible": false,
            "requires_headers": !source.headers.is_empty(),
        });
    }
    ...
    let response = tokio::time::timeout(Duration::from_secs(5), request.send()).await;
    let response = match response {
        Ok(Ok(response)) => response,
        Ok(Err(_error)) => {
            return serde_json::json!({
                "resolved": true,
                "probe_error": "upstream request failed",
                "browser_compatible": false,
                "requires_headers": !source.headers.is_empty(),
            });
        }
        Err(_) => {
            return serde_json::json!({
                "resolved": true,
                "probe_error": "upstream request timed out",
                "browser_compatible": false,
                "requires_headers": !source.headers.is_empty(),
            });
        }
    ...
    let content_disposition = response
        ...
        .and_then(|value| value.to_str().ok())
        .map(str::to_owned);
    let redirects = final_url != source.url;
    let media_type = detected_media_type(content_type.as_deref(), &final_url);
    let is_attachment = content_disposition
        .as_deref()
        .is_some_and(|value| value.to_ascii_lowercase().contains("attachment"));
    let browser_compatible = status.is_success()
        && !is_attachment
        && matches!(media_type, "mp4" | "hls" | "dash")
    ...
    serde_json::json!({
        ...
        "media": matches!(media_type, "mp4" | "hls" | "dash"),
        "browser_compatible": browser_compatible,
        "requires_headers": !source.headers.is_empty(),
        "reason": if !status.is_success() { Some("upstream response is not successful") } else if is_attachment { Some("download response") } else if !source.headers.is_empty() { Some("source requires request headers unavailable to a normal video element") } else if media_type == "unknown" { Some("content type is not a recognized MP4, HLS, or DASH type") } else { None::<&str> },
    })
}

async fn health() -> Json<serde_json::Value> {
    Json(serde_json::json!({"status":"ok","service":"moviebox-api"}))
}
async fn api_info() -> Json<serde_json::Value> {
    Json(
        serde_json::json!({"name":"MovieBox API","version":"1.0.0","endpoints":{"health":"/health","search":"/api/v1/search?q=...","title":"/api/v1/title/{id}","episodes":"/api/v1/title/{id}/episodes","stream":"/api/v1/stream/{id}","providers":"/api/v1/providers"}}),
    )
}
...
async fn search(State(state): State<AppState>, Query(params): Query<SearchQuery>) -> Response {
    ...
    if q.is_empty() || q.len() > 120 {
        return error(
            StatusCode::BAD_REQUEST,
            "INVALID_REQUEST",
            "Search query must be 1-120 characters",
        );
    }
    ...
    if successful_providers == 0 {
        return error(
            ...
            "UPSTREAM_ERROR",
            "All search providers failed or timed out",
        );
    }
...
async fn title(State(state): State<AppState>, Path(id): Path<String>) -> Response {
    let (provider, value) = match decode_id(&id) {
        Ok(v) => v,
        Err(r) => return r,
    };
...
async fn episodes(
    ...
    match tokio::time::timeout(
        ...
        state.service.details_typed(provider, &value),
    )
    .await
    {
        Ok(Ok(d)) => {
            ...
            Json(serde_json::json!({"title_id":encoded(provider,&value),"seasons":seasons}))
                .into_response()
        }
        Ok(Err(e)) => provider_error(e),
        Err(_) => error(
...
async fn stream(State(state): State<AppState>, Path(id): Path<String>) -> Response {
    let bytes = match URL_SAFE_NO_PAD.decode(&id) {
        Ok(v) => v,
        Err(_) => {
            return error(
                StatusCode::BAD_REQUEST,
    ...
    if parts.len() != 4 {
        return error(
            StatusCode::BAD_REQUEST,
            "INVALID_REQUEST",
            "Stream ID must identify provider, title, season, and episode",
        );
    }
    let provider = match ProviderKind::parse(parts[0]) {
        Some(p) => p,
        None => {
            return error(
                StatusCode::BAD_REQUEST,
    ...
    let season: usize = match parts[2].parse() {
        Ok(v) if v > 0 => v,
        _ => return error(StatusCode::BAD_REQUEST, "INVALID_REQUEST", "Invalid season"),
    };
    let episode: usize = match parts[3].parse() {
        Ok(v) if v > 0 => v,
        _ => {
            return error(
                StatusCode::BAD_REQUEST,
    ...
    match tokio::time::timeout(
        ...
        state
            .service
            .resolve_episode_playback_typed(provider, parts[1], season, episode),
    )
    .await
    {
        Ok(Ok(resolved)) => {
            let client = state.service.http_client().clone();
            let sources =
                futures::future::join_all(resolved.into_iter().map(|(release, source)| {
                    let client = client.clone();
                    async move {
                        let diagnostics = source_diagnostics(&client, &source).await;
                        let detected_type = diagnostics
                            .get("content_type")
                            .and_then(|value| value.as_str())
                            .map(|content_type| {
                                detected_media_type(Some(content_type), &source.url)
                            })
                            .unwrap_or("unknown");
                        let headers = source
                            .headers
                ...
                .await;
            Json(serde_json::json!({"id":id,"title":null,"sources":sources,"subtitles":[]}))
                .into_response()
        }
        Ok(Err(e)) => provider_error(e),
        Err(_) => error(
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let app = Router::new()
        ...
        .route("/api/v1/title/{id}", get(title))
        .route("/api/v1/title/{id}/episodes", get(episodes))
        .route("/api/v1/stream/{id}", get(stream))
        .with_state(state)
        .layer(cors)
    ...
    log::info!("moviebox-api listening on {addr}");
...
mod tests {
    ...
    #[test]
    fn ids_round_trip() {
        let id = encoded(ProviderKind::MovieBox, "abc:123");
        let (p, v) = decode_id(&id).unwrap();
        assert_eq!(p, ProviderKind::MovieBox);
        assert_eq!(v, "abc:123");
    }
}
