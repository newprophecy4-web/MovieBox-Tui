...
For Render, use build command `cargo build --release --locked`, start command `./target/release/moviebox-api`, and health check path `/health`. The stream endpoint uses each provider's existing playback flow at request time; 4KHDHub releases go through its validated resolver/preflight path, while provider-specific direct sources retain their existing behavior. The API does not store media or temporary stream URLs and exposes only safe playback headers. Users are responsible for complying with applicable law and provider terms.
